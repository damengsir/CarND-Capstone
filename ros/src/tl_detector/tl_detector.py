...
##==============================================================================
    ...
    def process_traffic_lights(self):
        ...
        if(self.pose_stamped):
            car_wp_idx = self.get_closest_waypoint(self.pose_stamped.pose.position.x, self.pose_stamped.pose.position.y)

        #TODO find the closest visible traffic light (if one exists)
            diff = len(self.waypoints_stamped.waypoints)
            for i, light in enumerate(self.lights): # the traffic light's information (position, number:8)
    ...
    def calculate_traffic_light_waypoints(self):
        ...
        if self.waypoints_stamped is not None and self.lights is not None and len(self.lights_wp) == 0:
            for i in range(len(self.lights)):
                ...
                self.stoplines_wp.append(self.get_closest_waypoint(stopline.position.x, stopline.position.y))
                self.lights_wp.append(self.get_closest_waypoint(self.lights[i].pose.pose.position.x, self.lights[i].pose.pose.position.y))
